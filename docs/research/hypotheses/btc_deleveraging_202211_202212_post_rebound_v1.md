...
- Hypothesis ID: `btc_deleveraging_202211_202212_post_rebound_v1`
...
- Event or state family: `POST_DELEVERAGING_REBOUND`
...
- Mechanism statement: Forced futures deleveraging can overshoot spot discovery, then rebound once liquidation pressure and open-interest compression have mostly cleared.
- Forced actor: Leveraged futures participants being forced out after a rapid unwind.
- Constraint: The unwind cannot continue indefinitely once aggressive flow exhausts and open interest has already flushed lower.
- Distortion: Temporary forced selling drives price below a more balanced post-liquidation level.
- Why this distortion should exist in this regime: `POSITIONING_UNWIND_DELEVERAGING` explicitly models forced unwind and recovery phases rather than generic mean reversion.
- Why the market should unwind rather than continue: Once liquidation flow decays and the rebound detector triggers, the marginal forced seller should be largely exhausted.
- Unwind path: deleveraging wave -> liquidation pressure decay -> post-deleveraging rebound -> short-horizon spot recovery.

## Tradable Expression

- Execution venue: `Binance spot`
- Instrument expression: Long `BTCUSDT` spot after `POST_DELEVERAGING_REBOUND`.
...
- Why this expression is simpler than the full mechanism: It avoids simulating derivatives positioning directly while testing whether the rebound is strong enough to appear in the spot leg.

## Entry / Exit

- Entry logic: Enter long one bar after `POST_DELEVERAGING_REBOUND`.
- Entry lag: `1`
- Exit logic: Fixed horizon exit at `12` bars.
- Invalidation: Continued downside after the rebound trigger or immediate renewed liquidation pressure.
...
- Execution realism risk: The rebound may exist only as a futures-microstructure bounce and not persist cleanly in spot after costs.
...
- What result would disconfirm the mechanism: No coherent long-side edge appears on the rebound trigger in the bounded slice.
...
- If `modify`: Keep the same regime and expression and switch only to the optional precursor `OI_FLUSH`.
