...
class CampaignController:
    ...
    def _propose_next_request(self) -> Optional[Dict[str, Any]]:
        ...
        if self.ledger_path.exists():
            ...
            if "trigger_payload" in df.columns:
                ...
                tested_events = {
                    event_id
                    for event_id in df["trigger_payload"]
                    .apply(_event_id_from_payload)
                    .dropna()
                    .astype(str)
                    if event_id
                }
        ...
        return {
            ...
            "trigger_space": {"allowed_trigger_types": ["EVENT"], "events": {"include": to_test}},
    ...
    def _update_frontier(self, ledger_df: pd.DataFrame):
        ...
        if not ledger_df.empty:
            ...
            fail_counts = (
                ledger_df[ledger_df["eval_status"].isin(["empty_sample", "insufficient_sample"])]
                .groupby("eid")
                .size()
            )
    ...
    def _get_partial_families(self, tested_events: Set[str]) -> Dict[str, float]:
        ...
        for eid, meta in events.items():
            ...
            if fam not in families:
                families[fam] = {"total": 0, "tested": 0}
            families[fam]["total"] += 1
            if eid in tested_events:
                families[fam]["tested"] += 1

        return {
            f: stats["tested"] / stats["total"]
            for f, stats in families.items()
            if 0 < stats["tested"] < stats["total"]
        }
